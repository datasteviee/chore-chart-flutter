-- Supabase performance advisor: use (select auth.uid()) instead of auth.uid()
-- in RLS so the value is not re-evaluated per row (initplan).
-- Also aligns families_insert / families_delete with the helper-function policies
-- if the database was migrated before those were folded into fix_members_rls_recursion.
